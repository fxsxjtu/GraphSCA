...
class AlgoConfig(BaseConfig):
    ...
    Args:
        ...
        adv_estimator (str): Advantage estimator type: "gae", "grpo", "gigpo", "gigpo_no_step_norm", "reinforce_plus_plus", etc.
        ...
        gigpo_gamma (float): Discount factor for step-level rewards in GiGPO algorithm.
            Controls how much future step rewards are valued. Range: (0, 1]
            - 0.5: Fast decay (default), only ~2-3 steps ahead matter
            - 0.7: Moderate decay, ~3-5 steps ahead matter
            - 0.9: Slow decay (standard RL), ~10+ steps ahead matter
            - 0.99: Very slow decay, considers very long horizons
            Default: 0.5
        gigpo_step_weight (float): Weight for step-level advantages in GiGPO.
            Controls the contribution of step rewards vs episode rewards.
            Default: 0.02
        gigpo_mode (str): Normalization mode for GiGPO step rewards.
            - "mean_norm": Only subtract mean (faster, default)
            - "mean_std_norm": Subtract mean and divide by std (more stable)
            Default: "mean_norm"
        gigpo_epsilon (float): Small constant for numerical stability in normalization.
            Default: 1e-6
    """
    ...
    # GiGPO-specific parameters
    gigpo_gamma: float = 0.5
    gigpo_step_weight: float = 0.02
    gigpo_mode: str = "mean_norm"
    gigpo_epsilon: float = 1e-6
    gigpo_advantage_scope: str = "token"
    gigpo_episode_advantage_scope: str = "token"
    gigpo_step_scatter_pool: str = "mean"
