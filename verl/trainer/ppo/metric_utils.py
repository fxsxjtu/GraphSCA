...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    Returns:
        A dictionary of metrics including:
            ...
            - critic/outcome_reward_raw/mean, max, min: Outcome-only reward before token broadcast
            - critic/step_reward_only/mean, max, min: Sum of GiGPO step rewards only
    ...
    correctness_reward = batch.non_tensor_batch.get("correctness_reward", None)
    if correctness_reward is not None:
        correctness_reward_tensor = torch.as_tensor(
            correctness_reward,
            dtype=sequence_score.dtype,
            device=sequence_score.device,
        ).reshape(-1)
        if correctness_reward_tensor.numel() == non_aborted_mask.numel():
            correctness_reward_tensor = correctness_reward_tensor[non_aborted_mask]
        correctness_reward_tensor = correctness_reward_tensor[torch.isfinite(correctness_reward_tensor)]
        if correctness_reward_tensor.numel() > 0:
            metrics.update(
                {
                    "critic/outcome_reward_raw/mean": torch.mean(correctness_reward_tensor).detach().item(),
                    "critic/outcome_reward_raw/max": torch.max(correctness_reward_tensor).detach().item(),
                    "critic/outcome_reward_raw/min": torch.min(correctness_reward_tensor).detach().item(),
                }
            )
    ...
    if "step_rewards" in batch.batch.keys():
        ...
        if step_rewards.numel() > 0:
            valid_step_rewards = torch.where(torch.isfinite(step_rewards), step_rewards, torch.zeros_like(step_rewards))
            per_seq_step_reward = valid_step_rewards.sum(dim=-1)
            if per_seq_step_reward.numel() == non_aborted_mask.numel():
                per_seq_step_reward = per_seq_step_reward[non_aborted_mask]
            per_seq_step_reward = per_seq_step_reward[torch.isfinite(per_seq_step_reward)]
            if per_seq_step_reward.numel() > 0:
                metrics.update(
                    {
                        "critic/step_reward_only/mean": torch.mean(per_seq_step_reward).detach().item(),
                        "critic/step_reward_only/max": torch.max(per_seq_step_reward).detach().item(),
                        "critic/step_reward_only/min": torch.min(per_seq_step_reward).detach().item(),
                    }
                )

    # GiGPO group/advantage diagnostics (populated by compute_gigpo_outcome_advantage)
    # Safe to import lazily: avoids circular import risk and keeps the cost to a dict lookup.
    try:
        from verl.trainer.ppo import core_algos as _core_algos

        gigpo_stats = getattr(_core_algos, "_last_gigpo_stats", None)
        if gigpo_stats:
            for key, value in gigpo_stats.items():
                # Sentinels: skip uninformative gate counts when the gate is disabled
                if key in ("step_reward_gate_valid_count", "step_reward_gate_total_count") and value == -1:
                    continue
                metrics[f"gigpo/{key}"] = value
    except Exception as _gigpo_metric_err:  # pragma: no cover - diagnostics must never break training
        print(f"[metric_utils] Failed to attach GiGPO stats: {_gigpo_metric_err}")
...
def compute_reward_extra_metrics(reward_extra_infos_dict: dict[str, Any]) -> dict[str, Any]:
    metrics = {}
    ...
    for key, values in reward_extra_infos_dict.items():
        ...
        if key in {
            ...
            metrics[f"reward_extra/{key}/max"] = float(np.max(finite))
            metrics[f"reward_extra/{key}/min"] = float(np.min(finite))
