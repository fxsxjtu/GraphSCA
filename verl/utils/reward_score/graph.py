...
def is_minimum_spanning_tree_weighted(graph, edge_list: list,) -> bool:
    ...
    if len(normalized_edges) != graph.number_of_nodes() - 1:
        return False

    T = nx.Graph()
    T.add_nodes_from(graph.nodes)
    T.add_edges_from(normalized_edges)
...
def extract_solution(solution_str):
    from verl.utils.reward_score.math_com import remove_boxed, last_boxed_only_string
...
if __name__ == "__main__":
    solution_str = "{1, 2, 3, 4 , 5, 6}"

    ground_truth = "{1, 2, 3, 4, 5, 6}"
    extra_info = {"task": "graph_dominating_set", "sample": "{1, 2, 3, 4, 5, 6}"}
