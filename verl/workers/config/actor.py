...
class PolicyLossConfig(BaseConfig):
    ...
    Args:
        loss_mode (str): Loss function mode. Options include 'vanilla', 'opsd',
            'opsd_future_kl', 'future_kl', 'observer_future_kl',
            'entity_anchored_grouped_kl', 'clip-cov', 'kl-cov', 'gpg'.
    ...
    # MGPO-style implicit-reward-adjusted PPO
    mgpo_beta: float = 0.01
    mgpo_center_by: str = "uid"
    mgpo_use_ref: bool = True
    mgpo_max_w: float = 5.0
    mgpo_max_kl: float = 10.0
    mgpo_max_entropy_ratio: float = 3.0

    # FIPO-style future-KL modulator (loss_mode=future_kl)
    ...
    # Observer-action Future-KL modulator (loss_mode=observer_future_kl).
    # This keeps outcome and observer-step advantages decoupled: Future-KL only
    # reweights the GiGPO step component over each observer action span.
    action_future_kl_decay_rate: float = 32.0
    action_future_kl_beta: float = 1.0
    action_future_kl_clip_low: float = 0.85
    action_future_kl_clip_high: float = 1.20
    action_future_kl_start: str = "after_action"
    action_future_kl_normalize: str = "sum"
    action_future_kl_direction: str = "none"
    action_future_kl_lambda: float = 1.0
    action_future_kl_renormalize_per_response: bool = False
    action_future_kl_reweight_episode: bool = False
    action_future_kl_reweight_episode_renormalize: bool = True
    action_future_kl_prefix_credit: bool = False
    action_future_kl_prefix_window: int = 64
    action_future_kl_prefix_decay: float = 0.96
    action_future_kl_prefix_budget_ratio: float = 0.15
    action_future_kl_prefix_min_action_delta: float = 1e-4
    action_future_kl_prefix_temperature: float = 0.003
    action_future_kl_credit_assignment: str = "off"
    action_future_kl_credit_window: int = 64
    action_future_kl_credit_target: str = "prefix"
    action_future_kl_credit_local_alpha: float = 0.2
    action_future_kl_credit_decay: float = 0.96
    action_future_kl_credit_temperature: float = 0.003
    action_future_kl_credit_min_support: float = 1e-6
    action_future_kl_credit_preserve_mass: bool = True

    # OPSD sampled-token self-distillation (loss_mode=opsd)
    opsd_loss_type: str = "sampled_pg"
    opsd_coef: float = 1.0
    opsd_adv_clip: float = 5.0
    opsd_center_advantage: bool = False
    opsd_use_ref_policy: bool = True
    opsd_teacher_prefix: str = ""
    opsd_teacher_prefix_builder: str = "verl.trainer.ppo.opsd_teacher_prefix.OPSDTeacherPrefixBuilder"
    opsd_teacher_prefix_max_chars: int = 1400
    opsd_graph_prefix_task_allowlist: str = ""
    opsd_graph_prefix_max_chars: int = 1000
    opsd_graph_prefix_fallback_algorithm_text: bool = True
    opsd_graph_prefix_include_negative_warning: bool = True
    opsd_skip_reward: bool = False
    # loss_mode=opsd_future_kl source signal. action_spans keeps only OPSD
    # teacher-gap signal on observer action spans before Future-KL transfer.
    opsd_future_kl_source: str = "action_spans"
    # Safety scale/cap after Future-KL transfer in loss_mode=opsd_future_kl.
    # cap_ratio=1.0 keeps the transferred absolute advantage mass no larger
    # than the action-span OPSD source mass, preventing credit transfer from
    # amplifying OPSD by orders of magnitude.
    opsd_future_kl_credit_scale: float = 1.0
    opsd_future_kl_credit_abs_cap_ratio: float = 1.0

    # Entity-anchored future-KL with GiGPO group-norm + additive composition
    # (loss_mode=entity_anchored_grouped_kl)
    anchor_decay_gamma: float = 0.978          # within-segment backward decay
    anchor_decay_lambda: float = 0.5           # cross-anchor forward decay
    anchor_alpha: float = 1.0                  # reward scale
    anchor_reward_clip: float = 1.0            # |reward| clip
    anchor_weight: float = 0.3                 # additive weight w
    anchor_protect_negative: bool = True       # zero positive reward on negative-adv tokens
    anchor_protect_symmetric: bool = False     # also zero NEGATIVE reward on positive-adv tokens
    anchor_size_1_strategy: str = "skip"       # skip / batch_mean / prompt_mean
    anchor_groupnorm_with_std: bool = False    # divide by group std
    anchor_compose_mode: str = "additive"      # additive / multiplicative
    anchor_mult_scale: float = 1.0             # tanh scale for multiplicative
    anchor_mult_max: float = 0.5               # max |1 - modulator| for multiplicative
