...
def _dispatch_compute_log_prob_with_rewards(worker_group, reward_fn, data, tokenizer):
    """Custom dispatch function for compute_log_prob_with_rewards.

    This function:
    - Broadcasts reward_fn and tokenizer to all workers
    - Chunks the data DataProto across workers
    """
    # Split the data across workers
    splitted_data_list = data.chunk(chunks=worker_group.world_size)

    # Broadcast reward_fn and tokenizer to all workers
    args = ([reward_fn] * worker_group.world_size, splitted_data_list, [tokenizer] * worker_group.world_size)
    kwargs = {}

    return args, kwargs


def _collect_compute_log_prob_with_rewards(worker_group, output):
    """Custom collect function for compute_log_prob_with_rewards."""
    from verl.single_controller.base.decorator import collect_dp_compute_data_proto
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                output, entropys, embeddings = self.actor.compute_log_prob(data=data, calculate_entropy=not is_lora)
            ...
            if embeddings is not None:
                # If embeddings are entity-level (2D), store in meta_info to avoid batch mismatch.
                if embeddings.dim() == 2:
                    meta_info["response_embeddings"] = embeddings
                else:
                    tensors["response_embeddings"] = embeddings
            output = DataProto.from_dict(tensors=tensors, meta_info=meta_info)
    ...
    @register(dispatch_mode={
        "dispatch_fn": _dispatch_compute_log_prob_with_rewards,
        "collect_fn": _collect_compute_log_prob_with_rewards
    })
    @DistProfiler.annotate(color="blue", role="actor_compute_log_prob_with_rewards")
    def compute_log_prob_with_rewards(self, reward_fn, data: DataProto, tokenizer):
        """Compute log probabilities and rewards in a streaming fashion.

        This method wraps the DataParallelPPOActor.compute_log_prob_with_rewards method
        to make it available through the RayWorkerGroup interface.

        Args:
            reward_fn: Reward function (will be broadcast to all workers)
            data: DataProto containing input data (will be chunked across workers)
            tokenizer: Tokenizer (will be broadcast to all workers)
        """
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        # Set up meta_info for the computation
        config_source = self.config.ref if is_lora else self.config.rollout
        # Preserve extract_embeddings and embedding_layer if already set
        extract_embeddings = data.meta_info.get("extract_embeddings", False)
        embedding_layer = data.meta_info.get("embedding_layer", 30)
        ...
        # Restore extract_embeddings and embedding_layer
        data.meta_info["extract_embeddings"] = extract_embeddings
        data.meta_info["embedding_layer"] = embedding_layer

        print(f"[DEBUG fsdp_workers] meta_info: extract_embeddings={extract_embeddings}, embedding_layer={embedding_layer}")

        # Perform streaming computation with rewards
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                (
                    log_probs,
                ...
                ) = self.actor.compute_log_prob_with_rewards(
                    data=data,
                    tokenizer=tokenizer,
                    reward_fn=reward_fn,
            ...
            print(
                "[DEBUG fsdp_workers] Received from dp_actor - "
                f"step_rewards: {step_rewards is not None}, "
                f"anchor_obs: {anchor_obs is not None}, "
                f"step_token_positions: {step_token_positions is not None}, "
                f"pure_step_reward_tensor: {pure_step_reward_tensor is not None}"
            )
            if step_rewards is not None:
                print(f"[DEBUG fsdp_workers] step_rewards shape: {step_rewards.shape}")
            if anchor_obs is not None:
                print(f"[DEBUG fsdp_workers] anchor_obs shape: {anchor_obs.shape}")
            if step_token_positions is not None:
                print(f"[DEBUG fsdp_workers] step_token_positions shape: {step_token_positions.shape}")
            if pure_step_reward_tensor is not None:
                print(f"[DEBUG fsdp_workers] pure_step_reward_tensor shape: {pure_step_reward_tensor.shape}")

            tensors = {"old_log_probs": log_probs, "streaming_reward_tensor": reward_tensor}
            if not is_lora:
                tensors["entropys"] = entropys
            # Add step_rewards for GiGPO if available
            if step_rewards is not None:
                tensors["step_rewards"] = step_rewards
                print(f"[DEBUG fsdp_workers] Added step_rewards to tensors")
            # Add pure_step_reward_tensor so the trainer can compute outcome-only episode advantage
            if pure_step_reward_tensor is not None:
                tensors["pure_step_reward_tensor"] = pure_step_reward_tensor
                print(f"[DEBUG fsdp_workers] Added pure_step_reward_tensor to tensors")

            non_tensors = {}
            # Add anchor_obs for GiGPO if available
            if anchor_obs is not None:
                non_tensors["anchor_obs"] = anchor_obs
                print(f"[DEBUG fsdp_workers] Added anchor_obs to non_tensors")
            if step_token_positions is not None:
                non_tensors["step_token_positions"] = step_token_positions
                print(f"[DEBUG fsdp_workers] Added step_token_positions to non_tensors")
            reward_extra_keys = []
            if reward_extra_infos:
                if hasattr(reward_fn, "get_reward_extra_keys"):
                    reward_extra_keys = list(reward_fn.get_reward_extra_keys())
                else:
                    reward_extra_keys = sorted(reward_extra_infos.keys())
                non_tensors.update(reward_extra_infos)
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during compute_log_prob_with_rewards", logger=logger)
