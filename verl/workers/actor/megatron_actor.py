...
class MegatronPPOActor(BasePPOActor):
    ...
    def make_minibatch_iterator(self, data: DataProto) -> Iterable[DataProto]:
        ...
        if self.config.use_kl_loss or loss_mode in ("mgpo", "opsd", "opsd_future_kl"):
        ...
        if self.has_multi_modal_inputs:
            data = data.select(select_keys, ["multi_modal_inputs"] + non_tensor_select_keys)
        else:
    ...
    def forward_backward_batch(
        ...
        def loss_func(output, data, meta_info):
            ...
            if not forward_only:
                ...
                observer_kwargs = {}
                if loss_mode in {"observer_future_kl", "opsd_future_kl"}:
                    observer_kwargs["advantages_episode"] = data.get("advantages_episode", None)
                    observer_kwargs["advantages_step"] = data.get("advantages_step", None)
                    observer_kwargs["step_token_positions"] = data.non_tensor_batch.get("step_token_positions", None)
