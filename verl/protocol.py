...
class DataProto:
    ...
    def chunk(self, chunks: int) -> list["DataProto"]:
        ...
        meta_info_lst = [self.meta_info for _ in range(chunks)]
        if self.meta_info is not None and "flat_entity_keys" in self.meta_info:
            if bsz_in_batch is None:
                raise ValueError("flat_entity_keys requires batch to be present for chunking.")
            from bisect import bisect_right

            prefix = np.cumsum(bsz_in_batch).tolist()
            chunk_flat_keys = [[] for _ in range(chunks)]
            for entry in self.meta_info["flat_entity_keys"]:
                sample_idx, entity_type, entity_id, span_positions, token_pos = entry
                if sample_idx < 0 or sample_idx >= prefix[-1]:
                    continue
                chunk_idx = bisect_right(prefix, sample_idx)
                start = prefix[chunk_idx - 1] if chunk_idx > 0 else 0
                chunk_flat_keys[chunk_idx].append(
                    (sample_idx - start, entity_type, entity_id, span_positions, token_pos)
                )

            meta_info_lst = []
            for i in range(chunks):
                meta_i = dict(self.meta_info)
                meta_i["flat_entity_keys"] = chunk_flat_keys[i]
                meta_info_lst.append(meta_i)

        output = []
        for i in range(chunks):
            output.append(
                type(self)(batch=batch_lst[i], non_tensor_batch=non_tensor_batch_lst[i], meta_info=meta_info_lst[i])
            )
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        # Handle special case for step_rewards which may have different sizes in dim=1
        if batch_lst[0] is not None and "step_rewards" in batch_lst[0].keys():
            # Find the maximum number of entities across all batches
            max_entities = max(batch["step_rewards"].shape[1] for batch in batch_lst if "step_rewards" in batch)

            # Pad step_rewards tensors to the same size
            for batch in batch_lst:
                if "step_rewards" in batch:
                    sr = batch["step_rewards"]
                    if sr.shape[1] < max_entities:
                        ...
                        padding = torch.full((sr.shape[0], max_entities - sr.shape[1]),
                                            ...
                                            dtype=sr.dtype,
                                            device=sr.device)
                        batch["step_rewards"] = torch.cat([sr, padding], dim=1)

        new_batch = torch.cat(batch_lst, dim=0) if batch_lst[0] is not None else None

        non_tensor_batch = list_of_dict_to_dict_of_list(list_of_dict=[d.non_tensor_batch for d in data])

        # Handle special case for anchor_obs which may have different sizes in axis=1
        if "anchor_obs" in non_tensor_batch:
            anchor_obs_list = non_tensor_batch["anchor_obs"]
            # Find the maximum number of entities across all batches
            max_entities = max(ao.shape[1] for ao in anchor_obs_list)

            # Pad anchor_obs arrays to the same size
            padded_anchor_obs_list = []
            for ao in anchor_obs_list:
                if ao.shape[1] < max_entities:
                    ...
                    padded_anchor_obs_list.append(ao_padded)
                else:
                    padded_anchor_obs_list.append(ao)

            non_tensor_batch["anchor_obs"] = np.concatenate(padded_anchor_obs_list, axis=0)

        # Handle special case for step_token_positions which may have different sizes in axis=1
        if "step_token_positions" in non_tensor_batch:
            step_token_positions_list = non_tensor_batch["step_token_positions"]
            max_entities = max(pos.shape[1] for pos in step_token_positions_list)

            padded_step_token_positions_list = []
            for pos in step_token_positions_list:
                if pos.shape[1] < max_entities:
                    ...
                    padded_step_token_positions_list.append(pos_padded)
                else:
                    padded_step_token_positions_list.append(pos.astype(object))

            non_tensor_batch["step_token_positions"] = np.concatenate(padded_step_token_positions_list, axis=0)

        # Concatenate other non-tensor batch items
        for key, val in non_tensor_batch.items():
            if key not in ("anchor_obs", "step_token_positions"):  # Already handled above
